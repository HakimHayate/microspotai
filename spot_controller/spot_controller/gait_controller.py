...
class GaitController():
    def __init__(self, controller_node, joints_names, thigh_foot=None, stride_length=0.06, 
                 stride_height=0.05, 
        ...
        self.thigh_foot_ = thigh_foot if thigh_foot is not None else None
    ...
    def get_foot_coordinate(self, phase):
        ...
        if phi < math.pi:
            z = (self.L / 2.0) * math.cos(phi + math.pi) + self.thigh_foot_[2]
            x =  - (self.H * math.sin(phi)) + self.thigh_foot_[0]
            
        else:
            stance_progress = (phi - math.pi) / math.pi 
            z = (self.L / 2.0) - (self.L * stance_progress) + self.thigh_foot_[2]
    ...
    def trot_gait(self, thigh_foot=None, reset=False):
        if reset:
            self.time_elapsed_ = 0
        self.thigh_foot_ = thigh_foot
